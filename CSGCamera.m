...
{
    // needs to clean up on item failures
...
//    NSLog(@"boundsRect=(%d, %d, %d, %d)", boundsRect.top, boundsRect.left, boundsRect.bottom, boundsRect.right);
    ...
    // Start frame timer
    frameTimer = [[NSTimer scheduledTimerWithTimeInterval:0.0 
			target:self 
...
{   
	...
	if (component) {
		SGStop(component);
		// clear the data proc -- Sprouted addition
		SGSetDataProc(component,NULL,0);
	}
	...
	/* Begin Sprouted Additions */
	// close the channel
	if ( channel ) {
		
		theErr = SGSetChannelUsage(channel,0);
		if ( theErr != noErr ) {
			NSLog(@"SGSetChannelUsage() returned %ld", theErr);
		}
		
		theErr = SGDisposeChannel(component, channel);
		if ( theErr != noErr ) {
			NSLog(@"SGDisposeChannel() returned %ld", theErr);
		}
		
		channel = NULL;
	}
	
	/* End Sprouted Additions */
...
- (BOOL)_setupDecompression;
{
    ...
    theErr = DecompressSequenceBegin(&decompressionSequence, 
			imageDesc, 
			gWorld, 
			NULL, 
			NULL, 
